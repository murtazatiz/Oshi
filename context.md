...
│   │   ├── services/             # apiClient, analytics, offline queue, purchases
...
│   ├── src/
│   │   ├── routes/               # auth, saves, categories, users, notifications, subscriptions, engagement
│   │   ├── services/             # aiService, metadataService, youtubeService, smartSortService, etc.
│   │   ├── workers/              # BullMQ: aiProcessor, deadLinkChecker, notificationScheduler
│   │   ├── cron/                 # Daily reminders, monthly save-count reset
│   │   ├── middleware/           # auth, rateLimiter, errorHandler
│   │   ├── queue/                # BullMQ queue setup
│   │   └── lib/                  # prisma, redis
│   └── prisma/schema.prisma      # Database schema
│
├── .cursor/rules/                # Cursor IDE coding standards (core, backend, react-native, state, supabase)
...
| Auth | Supabase Auth (JWT in Expo SecureStore) |
...
| Animations | React Native Animated (core) — Reanimated 2 planned per PRD |
| Haptics | expo-haptics |
| Share | expo-share-intent + custom iOS share extension plugin |
| Offline | AsyncStorage queue (`offlineQueue.ts`) |
...
| AI | OpenAI GPT-4o-mini (primary categorisation) |
...
| Cron | node-cron (daily reminders, monthly resets, dead link checks) |
| Error monitoring | Sentry (`@sentry/node`) |
| Analytics | PostHog (`posthog-node`) |
| Payments webhook | RevenueCat webhook handler |
...
| Auth | Supabase Auth (Apple, Google, Email/Password OAuth) |
| File storage | Supabase Storage (`avatars` bucket, signed URLs) |
| Realtime | Supabase Realtime |
| Backend hosting | Railway.app (auto-deploy on push to `main`) |
...
│  BullMQ queues: ai-processing, notification-scheduler    │
└──────────────────────┬──────────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────────┐
...
3. Backend validates JWT, checks save limit (free tier: 20/month), creates `saves` record with `processing_status: 'pending'`
4. Backend enqueues BullMQ `ai-processing` job → returns `{ id, status: 'pending' }` in <300ms
...
1. BullMQ `notification-scheduler` runs every minute
2. Queries users whose `reminder_time` matches current minute
...
| Table | Purpose |
|-------|---------|
| `users` | Profile, subscription status, reminder settings, streak, push token |
...
### Key Enums (enforced by DB CHECK constraints)
...
└── Main (authenticated + onboarding complete)
    ...
    │   ├── SearchTab  → ImportScreen  (tab label: "Import", icon: ⬇)
...
| `oshi://notifications/enable` | Opens OS notification settings |
...
**`GET /saves` is only called on:**
- App load
- Pull-to-refresh
- Header refresh button tap
- After a new save is added

**Category tab switches, search, and filter chips are purely client-side** — no API call per category.
...
**Decision:** `POST /saves` creates a pending record and returns immediately (<300ms). BullMQ worker handles metadata fetch + OpenAI call asynchronously. Supabase Realtime pushes the update to the app.
...
**Decision:** Explicit hostname checks in both `metadataService.ts` and `saves.ts` route handler:
...
**Decision:** Repurpose the middle tab as **Import** (`ImportScreen.tsx`) with two sub-tabs:
...
Duplicate detection: `POST /saves` returning `{ duplicate: true }` counts as skipped.
...
Per cursor rules: JWT/session stored in Expo SecureStore. Zustand persist only for non-sensitive UI preferences (sort option, view mode).
...
| `GET /saves` returned 422 | App sent `limit=500` (string); Zod `max(50)` | Updated schema to `z.coerce.number().max(500)` |
...
- [x] Smart sort algorithm (server-side, cached)
- [x] Dead link checker (nightly cron)
...
- [ ] Reanimated 2 animations (currently using RN core Animated)
...
| **Pro** | Unlimited saves, unlimited categories, AI smart sort, morning digest, per-category reminder overrides |
...
Dark mode overrides exist for `background`, `surface`, `textPrimary`, `textSecondary`, `border`.
...
## 13. Coding Standards (Cursor Rules)

### TypeScript (`.cursor/rules/core.mdc`)
...
- SafeAreaProvider + SafeAreaView on every screen
...
cd app
npm install
npx expo start       # Expo dev server — http://localhost:8081
```
...
- Default branch: `main`
- Initial commit pushed June 2026
...
# Backend deploys automatically on push to main via Railway
```
...
| `.cursor/rules/*.mdc` | Cursor IDE coding standards per layer |
...
- **System prompt**: Built dynamically with user's actual category names
...
| Spotify | Spotify Embed API | Full |
